...
Results are written to on the runner directory:
...
Performance / Accuracy (IC, KWS, VWW, AD):   Host  <--------------->  DUT
...
For more details about how test scripts work, see the
[Test Scripts `tests.yaml`](#test-scripts-testsyaml) section below.
...
Copy the contents of `tiny/benchmark/runner/sww_data_dir` into the sub-directory `sww01` under the data directory.
This directory includes the main JSON truth files as well as smaller debugging files.
