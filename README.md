...
## Runner Transition Notice (v1.3)

The EEMBC EnergyRunner is accepted for KWS, VWW, IC, and AD. Use this MLCommons Runner for current runner-based workflows. The EEMBC runner does not support the Streaming Wake Word benchmark. See `../MLPerfTiny_Rules.adoc`.

---

## Overview

The runner software in this directory coordinates the execution of the MLPerf Tiny benchmarks. It communicates with the Device Under Test (DUT) either directly or through the interface board, and optionally with the power monitoring device (for example LPM01A or JouleScope JS220) to transmit test data and collect results.

The outputs are recorded to `./sessions/YYYYMMDD_HHMMSS/`, where `YYYYMMDD_HHMMSS` is a timestamp. Messages to and from the connected devices are recorded to `log.txt` in that directory. The results for submission are in `results.txt`, and additional results data is recorded in `results.json` in case additional analysis is desired.
...
This section gets a new user running quickly, while the later sections explain the full setup in detail.

### Supported Benchmarks

| ID    | Task                 | Mode                    | Model            | Quality Target |
| ----- | -------------------- | ----------------------- | ---------------- | -------------- |
| ic01  | Image Classification | Single-stream           | ResNet           | 85% (Top 1)    |
| kws01 | Keyword Spotting     | Single-stream, Offline  | DS-CNN           | 90% (Top 1)    |
| vww01 | Visual Wake Words    | Single-stream           | MobileNet        | 80% (Top 1)    |
| ad01  | Anomaly Detection    | Single-stream           | Deep AutoEncoder | 0.85 (AUC)     |
| sww01 | Streaming Wake Word  | Streaming               | 1D DS-CNN        | ⇐ 8 FP, ⇐ 8 FN |

### First Test

For a first trial in performance or accuracy mode:

1. Install the Python dependencies.
2. Connect the DUT directly to the host PC via USB.
3. Prepare the dataset root directory.
4. Run one of the commands below.

For example using the Image Classification setup:

```bash
# Performance
python main.py --dataset_path=/path/to/datasets/ --test_script=tests_performance.yaml --device_list=devices_kws_ic_vww.yaml --mode=p

# Accuracy
python main.py --dataset_path=/path/to/datasets/ --test_script=tests_accuracy.yaml --device_list=devices_kws_ic_vww.yaml --mode=a
```

Results are written to the runner directory:
...
1. Detects the configured devices from `devices.yaml`.
2. Opens the DUT connection either directly or through the interface board.
3. Loads the selected test script from `tests_*.yaml`.
4. Sends commands and data to the DUT.
5. Collects accuracy / performance / energy / streaming results.
6. Stores logs and results in a timestamped session directory.

### Communication Paths

#### Host <==> DUT

For performance and accuracy tests, the host can be directly connected to the DUT. The baud rate is configurable by setting the `baud` parameter in the appropriate element in `devices.yaml`. The baud rate can either be a single integer baud rate, which will be used for all tests, or a dictionary with keys `energy`, `performance`, and `accuracy`.

If different benchmarks require different baud rates or hardware arrangements, you can use different device files, for example `devices_ad.yaml` and `devices_sww.yaml`, and pass them on the command line using `--device_list`.

#### Host <==> Interface Board

The host communicates with the interface board via a virtual serial port over USB.

#### Interface Board <==> DUT

For energy measurements, the interface board communicates with the DUT over UART.

For Streaming Wake Word, the interface board is also responsible for streaming audio to the DUT over I2S and collecting event timing signals from the DUT.

### System Architecture Summary

**Performance / Accuracy (IC, KWS, VWW, AD):**  
Host  <--------------->  DUT

**Energy (IC, KWS, VWW, AD):**  
Host  <---------->  Interface Board  <---------->  DUT  
Host  <----------->  Power Monitor

**Streaming Wake Word (SWW):**  
Host  <-------->  Interface Board  <-------->  DUT  
Host  <-------->  Power Monitor  
Interface Board  <--------> DUT
...
Specified by the command line option `-d` or `--device_list`, this yaml file lists all of the devices used in the test, specifically the energy monitor board, the DUT, and the interface board. Defaults to `devices.yaml`, which is the default/example file provided in the repo. Each device has several attributes:

- **name** Largely arbitrary but should be unique within the yaml file.
- **type** Either `interface`, `power`, or `dut`.
- **preference** If there are multiple interface boards connected and specified in the file, the one with the highest preference will be used.
...
- **usb** A pair of hex values specifying the VID and PID of the device's USB connection.

### `tests.yaml`

For more details about how test scripts work, see the [Test Scripts `tests.yaml`](#test-scripts-testsyaml) section below.
...
For performance and accuracy tests, the host can be directly connected to the DUT. The baud rate is configurable by setting the `baud` parameter in the appropriate element in `devices.yaml`. The baud rate can either be a single integer baud rate, which will be used for all tests, or a dictionary with keys `energy`, `performance`, and `accuracy`. The default `devices.yaml` included in this repo uses the dictionary for the L4R5zi reference DUT. If you need different baud rates for different benchmarks (e.g. for anomaly detection vs. streaming wakeword) you can use different device files (e.g. `devices_ad.yaml` and `devices_sww.yaml`) and pass them on the command line using the `--device_list` flag.
...
4. The test you run will be determined by the `sww01.truth_file` value in `tests.yaml`. It is recommended to start with the short test (`truth_file: sww_short_test.json`), a 10-second wav file. The current model has 2 true positive and 1 false negatives on this file.
...
Datasets should be placed under a common root directory passed to `--dataset_path`.
...
For IC, KWS, VWW, and AD, use the benchmark-specific training and preprocessing flow to generate the dataset files expected by the runner.

### Streaming Wake Word Data

Copy the contents of `tiny/benchmark/runner/sww_data_dir` into the sub-directory `sww01` under the data directory. This directory includes the main JSON truth files as well as smaller debugging files.
...
The host-interface baud rate defaults to 115200 but can be set by an optional `baud` setting in the `devices.yaml` file. As a last-resort debug method, the host-interface baud rate can also be set by modifying `io_manager.py`; update **line 6** to reflect the **computer → interface** baud rate.
...
This typically means that the host or interface board cannot communicate with the DUT, because the `name` command is the first command sent to the DUT. Make sure that the DUT is correctly powered, the baud rate is correctly specified (noting that you may specify different baud rates for energy/performance/accuracy modes), and that the UART wires are correctly connected if not connecting directly to the DUT. If you're using the L4R5zi reference board, remember that the Tx/Rx lines are labeled backwards.
...
2. This will queue up another terminal for formatting an SD device, then run the following commands:
...
2. To reduce the number of connections, you can run the test without the power board and with the host connected directly to the DUT's USB virtual UART, although the interface board is still necessary for delivering I2S signals. The runner will detect the connection and issue commands directly to the DUT rather than going through the interface board. This setup will also let you use the step debugger in the DUT. In this arrangement, make sure that the UART wires are disconnected so they do not contend with the USB UART and make sure the IDD jumper is in place, since the DUT is being powered through the USB connection.
